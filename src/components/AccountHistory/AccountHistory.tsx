...
export const AccountHistory: React.FC<AccountHistoryProps> = ({ accounts }) => {
  ...
  const { isEnabled, toggleEnabled, userId } = useAccountHistory();
    ...
    return (
      <div className={styles.accountHistory}>
        ...
        <div className={styles.disabledMessage}>
          ...
          <button className={styles.enableButton} onClick={() => toggleEnabled(true)}>
  ...
  return (
    <div className={styles.accountHistory}>
      <div className={styles.header}>
        <h2 className={styles.title}>{t('account_history')}</h2>
        <button className={styles.toggleButton} onClick={() => toggleEnabled(false)}>
          {t('disable_history')}
        </button>
      </div>
      ...
      <div className={styles.content}>
        ...
        <div className={styles.tabContent}>
          {activeTab === 'viewer' && <AccountHistoryViewer accounts={accounts} />}
          {activeTab === 'settings' && <AccountHistorySettings accounts={accounts} />}
        </div>
      </div>
    </div>
  );
